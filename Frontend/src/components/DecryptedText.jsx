...
export default function DecryptedText({
        ...
        if (sequential) {
          if (direction === 'forward') {
            if (prevRevealed.size < text.length) {
              ...
              setIsDecrypted(true)
              setTimeout(() => onDecryptCompleteRef.current?.(), 0)
              return prevRevealed
            }
          }
  ...
  const [prevTextProps, setPrevTextProps] = useState(
    animateOn === 'click' ? null : { text, animateOn }
  )

  if (
    prevTextProps === null ||
    prevTextProps.text !== text ||
    prevTextProps.animateOn !== animateOn
  ) {
    setPrevTextProps({ text, animateOn })
    if (animateOn === 'click') {
      encryptInstantly()
    } else {
      setDisplayText(text)
