...
void main() {
  ...
  // Running gradient: hue flows over time and spreads a little across the
  // swarm's field intensity, so the whole cursor continuously shifts colour.
  float hue = fract(uTime * uSpeed + f * uSpread);
...
const SwarmCursor = ({
  ...
  useEffect(() => {
    ...
    // In overlay mode the container is pointer-events:none, so track pointer on
    // the window instead of the container itself.
    const target2 = overlay ? window : container;
    target2.addEventListener('pointermove', onMove, { passive: true });
    target2.addEventListener('pointerdown', onDown);
    if (overlay) {
    ...
    return () => {
      ...
      ro.disconnect();
      target2.removeEventListener('pointermove', onMove);
      target2.removeEventListener('pointerdown', onDown);
      if (overlay) {
  ...
  return (
    <div
      ref={containerRef}
      className={`swarm-cursor${overlay ? ' swarm-cursor--overlay' : ''}${className ? ` ${className}` : ''}`.trim()}
      style={overlay ? { ...style, zIndex } : style}
      {...rest}
    >
